...
void body::sumVector(std::deque<body> dq) {
    ...
    if (!miilive)return;
    float tmplhitx[dq.size()] = {0};
    float tmplhity[dq.size()] = {0};
    bool tmpishit[dq.size()] = {0};
    int tmphitid[dq.size()] = {0};
    llhit = false;
    if (this->islon) {
            ...
            if (animbulletbtn) {
                ...
                if (bulletanimtimer > 1) {
                    for (int ii = 0; ii < maxbullets; ii++) {
                        ...
                        if (!bulletalive[ii]) {
                            //wdrt
                            float tm[2] = {137.5 + shippos[0] - shipthwgrt[0], 137.5 + shippos[1] - shipthwgrt[1]};
                            float tm2[2] = {238.5 + shippos[0] - shipthwgrt[0], 238.5 + shippos[1] - shipthwgrt[1]};
                            float ret[2] = {0};
